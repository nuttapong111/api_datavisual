...
const TopCountries: React.FC = () => {
  ...
  const fetchTopCountries = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await DataService.getTopCountries(selectedYear, 20);
    ...
    return (
      <div className="top-countries">
        ...
        <div className="error-state">
          <p>{error}</p>
          <button onClick={fetchTopCountries} className="retry-button">
            ลองใหม่
          </button>
        </div>
      </div>
              ...
              {data.map((item, index) => (
                <tr key={item.country}>
                  <td className="looker-rank-cell">
                    <span className={`looker-rank-badge ${index < 3 ? 'looker-top-three' : ''}`}>
                      {index + 1}
                    </span>
                  </td>
